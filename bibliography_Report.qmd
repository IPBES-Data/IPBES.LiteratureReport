...
In the bibliographic analysis, we can only analyse the references in [OpenAlex](https://OpenAlex.org){target="_blank"} as the bibliographic metadata alanised is not in the Zotero liubrary but retrieved from [OpenAlex](https://OpenAlex.org){target="_blank"}.

## Publication types

Works on Zenodo as well as on [OpenAlex](https://OpenAlex.org){target="_blank"} are of different types. Here we show these separate.
...
The types entered in the Zenodo library and the types as retrieved from [OpenAlex](https://OpenAlex.org){target="_blank"} are not linked, therefore they can differ, This can indicate problems but not necessarily has to. These cases need to be carefully evaluated and here only a starting point is given. 

`NA` in [OpenAlex](https://OpenAlex.org){target="_blank"} is likely caused by the doi not found in [OpenAlex](https://OpenAlex.org){target="_blank"} (not in there, not valid, non-standard DOI in Zenodo). `NA` in Zotero is linked to non-standard DOI entries (i.e. with `http`) whcih can therefore not be linked to [OpenAlex](https://OpenAlex.org){target="_blank"} data. If all DOIs are in standard format, there should be no NAs in this Table in the column Ferom Zenodo.
...
This is checked by using the [OpenAlex](https://OpenAlex.org){target="_blank"} retrieved works. Therefore it is li=mited to the works that are on [OpenAlex](https://OpenAlex.org){target="_blank"}. At the moment, only references with a DOI were retrieved from [OpenAlex](https://OpenAlex.org){target="_blank"}.
...
Based on the data entered in the [bibliography on Zenodo](`r bib_data$bibliography$url`){target="_blank"}
...
In this section we will take a closer look at the countries from which ih the authors ot the publication come. THe data comes from [OpenAlex](https://OpenAlex.org){target="_blank"} and includes all authoirs without weighting.
