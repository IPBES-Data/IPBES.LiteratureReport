...
format:
    html:
        ...
        #     gutter-width: 1.5rem    
params:
    bib_name: "IPBES_IAS_2352922"
    targets_store: "/Users/rainerkrug/Documents_Local/git/IPBES_data_tsu/Packages/IPBES.LiteratureReport/_targets"
---
...
- [This Report](`r paste0("https://ipbes-data.github.io/IPBES.LiteratureReport/output/IPBES.", bib_data$bibliography$name, ".bibliography_report.html")`)
...
- **ISBNss**: `r bib_data$metrics$zotero_isbns$count` (`r round(bib_data$metrics$zotero_isbns$pc, digits = 2)`%) - `r length(bib_data$metrics$zotero_isbns$duplicate)` duplicates
- **ISBNss**: `r bib_data$metrics$zotero_issn$count` (`r round(bib_data$metrics$zotero_issns$pc, digits = 2)`%) - `r length(bib_data$metrics$zotero_issns$duplicate)` duplicates
...
All DOIs should be given as only the DOI without the resolver, i.e. `not starting with `http`. The following table shows all entries which do have a non-copnform DOI: 
...
data.frame(
  ...
  knitr::kable(
    caption = "DOIs in Zenodo which ar not in [OpenAlex](https://OpenAlex.org)",
    escape = FALSE
  )
```
...
This is disabled at the moment and still needs o=to be implemented.
...
In the bibliographic analysis, we can only analyse the references in [OpenAlex](https://OpenAlex.org) as the bibliographic metadata alanised is not in the Zotero liubrary but retrieved from [OpenAlex](https://OpenAlex.org).

## Publication types

Works on Zenodo as well as on [OpenAlex](https://OpenAlex.org) are of different types. Here we show these separate.
...
### Zenodo versus [OpenAlex](https://OpenAlex.org) Types

The types entered in the Zenodo library and the types as retrieved from [OpenAlex](https://OpenAlex.org) are not linked, therefore they can differ, This can indicate problems but not necessarily has to. These cases need to be carefully evaluated and here only a starting point is given. 

`NA` in [OpenAlex](https://OpenAlex.org) is likely caused by the doi not found in [OpenAlex](https://OpenAlex.org) (not in there, not valid, non-standard DOI in Zenodo). `NA` in Zotero is linked to non-standard DOI entries (i.e. with `http`) whcih can therefore not be linked to [OpenAlex](https://OpenAlex.org) data. If all DOIs are in standard format, there should be no NAs in this Table in the column Ferom Zenodo.
...
bib_data$metrics$types$comparison |>
  knitr::kable(
    col.names = c("Type from Zenodo", "Type from [OpenAlex](https://OpenAlex.org)", "count"),
    caption = "Table giving the combination of types from Zenodo as well as [OpenAlex](https://OpenAlex.org) and their frequency"
  )
```
...
This is checked by using the [OpenAlex](https://OpenAlex.org) retrieved works. Therefore it is li=mited to the works that are on [OpenAlex](https://OpenAlex.org). At the moment, only references with a DOI were retrieved from [OpenAlex](https://OpenAlex.org).
