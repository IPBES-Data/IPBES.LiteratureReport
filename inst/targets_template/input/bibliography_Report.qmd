...
Cleanliness of a bibliography is defined here as the quality of the bibliographic data in the bibliography. It does not concern any topical measures, only measures related to the bibliographic data in the bibliography.

The analysis is at the moment only done for DOIs as corresponding measures are not available at the moment for ISBNs and ISSNs and duplicate ISBNs and ISSNs can be correct as different chapters of a book might be in the bibliography.
...
- **DOIs**: `r metrics$zotero_doi$count` (`r round(metrics$zotero_doi$pc, digits = 2)`%) - `r length(metrics$zotero_doi$duplicate)` duplicates
- **ISBNss**: `r metrics$zotero_isbn$count` (`r round(metrics$zotero_isbn$pc, digits = 2)`%)
- **ISBNss**: `r metrics$zotero_issn$count` (`r round(metrics$zotero_issn$pc, digits = 2)`%)

## DOIs in Zotero

The following sections provide some more detailed analysis of the bibliography in Zenodo. The tables all contain clickable links which, when clicked, show the respective article in the Zotero bibliography.
...
if (length(metrics$zotero_doi$duplicate) > 0) {
  data.frame(
    doi = metrics$zotero_doi$duplicate,
    link = paste0(
      bibliography$url,
      "/items/",
      names(metrics$zotero_doi$duplicate)
    )
  ) |>
    dplyr::mutate(
      sort = doi,
...
http <- grep("http", metrics$zotero_doi$id_raw)
data.frame(
  doi = metrics$zotero_doi$id_raw[http],
  link = paste0(
    bibliography$url,
    "/items/",
    names(metrics$zotero_doi$id_raw)[http]
  )
) |>
  dplyr::mutate(
    doi = paste0('<a href="', link, '" target="_blank">', doi, "</a>"),
...
Of these `r (length(metrics$dois_comp$zotero_not_in_oa) - length(metrics$zotero_doi$dois_valid))` are not valid. These are:
...
if (length(metrics$zotero_doi$not_valid) > 0) {
  data.frame(
    doi = metrics$zotero_doi$not_valid,
    link = paste0(
      bibliography$url,
      "/items/",
      names(metrics$zotero_doi$not_valid)
    )
  ) |>
    dplyr::mutate(
      doi = paste0('<a href="', link, '" target="_blank">', doi, "</a>"),
...
Of the `r length(metrics$zotero_doi$unique)` unique DOIs in the library, `r length(metrics$dois_comp$zotero_not_in_oa)` (`r round(100 * length(metrics$dois_comp$zotero_not_in_oa) / metrics$zotero_doi$count, digits = 2)`%) are in not in [OpenAlex](https://OpenAlex.org){target="_blank"}. This includes the non-valid DOIS, which are filtered out in the table below.
...
data.frame(
  doi = metrics$dois_comp$zotero_not_in_oa,
  link = paste0(
    bibliography$url,
    "/items/",
    names(metrics$dois_comp$zotero_not_in_oa)
  )
) |>
  dplyr::mutate(
    doi = paste0('<a href="', link, '" target="_blank">', doi, "</a>"),
